...
def _settings():    
    ...
    column2 = [
            ...
            [sg.InputCombo(('Human Player', 'RandomPlayer', 'GreedyPlayer', 'GreedyForker', 'ExploreExploitPlayer', 'DirectionsPlayer'), size=(35, 10),default_value='GreedyPlayer', key="player1", enable_events=True)],
            ...
            [sg.InputCombo(('Human Player', 'RandomPlayer', 'GreedyPlayer', 'GreedyForker', 'ExploreExploitPlayer', 'DirectionsPlayer', 'None'), size=(35, 10), default_value='GreedyPlayer', key="player2", enable_events=True)],
    ...
    while (True):
        ...
        if event == 'Submit':
            print(values)
            players = []
            if values['player1'] == "ExploreExploitPlayer" or values['player1']=="DirectionsPlayer":
                players.append({"type":values["player1"], "genes":values["gene1"], "gene_length":int(values["gene1l"])})
            else:
                players.append({"type":values["player1"]})
            if values['player2'] == "ExploreExploitPlayer" or values['player2']=="DirectionsPlayer":
